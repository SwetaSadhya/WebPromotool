...
$Seccust[] = new SecCustomer($secondaryCustomerID, $countryID, $secCustomerClientID, $secCustomerName, $secSecondaryCustomerClientName, $customerID, $cusCustomerClientCode, $secCustomerClientCode, $secGroup1ID, $secGroup1Desc, $secGroup2ID ,$secGroup2ClientID);    
//---------------------------------------Cust Temp Table--------------------------------------------------
$SeccustTemQuery='INSERT INTO '.SECCUST_TEMP_TABLE.'(secondaryCustomerID, countryID, secCustomerClientID, secCustomerName, secSecondaryCustomerClientName, customerID, cusCustomerClientCode, secCustomerClientCode, secGroup1ID, secGroup1Desc, secGroup2ID ,secGroup2ClientID, isActive)VALUES
('.$secondaryCustomerID.', "'.$countryID.'","'.$secCustomerClientID.'","'.$secCustomerName.'","'.$secSecondaryCustomerClientName.'","'.$customerID.'","'.$cusCustomerClientCode.'","'.$secCustomerClientCode.'","'.$secGroup1ID.'","'.$secGroup1Desc.'","'.$secGroup2ID.'","'.$secGroup2ClientID.'","1")';
...
//--------------------------------Group1----------------------------------------------
$tbl1=new Model;
$sql1="SELECT countryID,secGroup1ID,secGroup1Desc FROM ".SECCUST_TEMP_TABLE." ORDER BY  secGroup1ID";
...
{						
 foreach($res1 as $row1){
		$tbl1->_table_name= SECCUST_G1_TABLE;
			...
			if($g1Count == 0){
			$g1Query="INSERT INTO ".SECCUST_G1_TABLE."(secGroup1ID, countryID, secGroup1Desc)VALUES('".$row1->secGroup1ID."', '".$row1->countryID."','".$row1->secGroup1Desc."')";
			//echo $g1Query;
			//$tbl1->query($g1Query);	
			}
	}
...
//---------------------------------------Group2--------------------------------------------------
$tbl2=new Model;
$sql2="SELECT countryID,secGroup2ID,secGroup2ClientID FROM ".SECCUST_TEMP_TABLE." ORDER BY  secGroup2ID";
...
{						
 foreach($res2 as $row2){
		...
		if($g2Count == 0){
		$g2Query="INSERT INTO ".SECCUST_G2_TABLE."(secGroup2ID, countryID, 	secGroup2ClientID)VALUES('".$row2->secGroup2ID."', '".$row2->countryID."', '".$row2->secGroup2ClientID."')";
		//$tbl2->query($g2Query);
		}
	}
}
