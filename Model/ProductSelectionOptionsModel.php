...
class Model_ProductSelectionOptions extends Model_DbConnection {
	...
	// get Products
	public function getProduct($prdsearchTxt) {
	
	if(isset($prdsearchTxt) && $prdsearchTxt!=""){
	$condition =" and  ProductName like '%".$prdsearchTxt."%'";
	$query = "SELECT * FROM tblProduct where isActive = '1'  ".$condition."  GROUP BY ProductID";
	}else{
	...
	$Prd = new Model_Product($row['ProductID'],$row['countryID'],$row['prdProductClientCode'],$row['ProductName'],$row['prdCreatedOn'],$row['prdGST'],$row['prdNetWeight'],$row['prdNetVolume'],$row['prdGroup1ID'],$row['prdGroup2ID'],$row['prdGroup3ID'],$row['prdGroup4ID'],$row['prdGroup5ID'],$row['prdGroup6ID'],$row['prdGroup7ID'],$row['prdGroup8ID'],$row['prdGroup9ID'],$row['prdGroup10ID'],$row['prdGroup11ID'],$row['prdGroup12ID'],$row['prdGroup13ID'],$row['prdGroup14ID'],$row['prdGroup15ID'],$row['prdGroup16ID'],$row['prdGroup17ID'],$row['prdGroup18ID'],$row['importLogID'],$row['isActive']);
	$Products[] = $Prd;
	}
